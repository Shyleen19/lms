...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (path.endsWith('course-content.html')) {
        fetchCourses();
    } else if (path.endsWith('leader-board.html')) {
        fetchLeaderboard();
    } else if (path.endsWith('my-courses.html')) {
        fetchMyCourses();
    }
});
...
function fetchMyCourses() {
    fetch('/courses')
        .then(response => response.json())
        .then(data => {
            const myCoursesList = document.getElementById('courses-list');
            data.forEach(course => {
                const listItem = document.createElement('li');
                listItem.textContent = course.name;
                CoursesList.appendChild(listItem); // Append listItem to myCoursesList
            });
        })
        .catch(error => {
